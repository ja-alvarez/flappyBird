...
img_bird = Image.open('images/bird.png') #Cargar imagen
img_bird = img_bird.resize((100, 100)) #redimensionar imagen
img_bird = ImageTk.PhotoImage(img_bird) #Convierte imagen a formato compatible
...
def move_pipe():
    ...
    if canvas.coords(pipe_top)[0] < -100:
        score +=1
        speed +=1
    ...
    if 0 < canvas.coords(pipe_down)[0]<160:
        ...
        mixer.music.load('audio/point.wav')
        mixer.music.play(loops=0)
...
btn_reset = tk.Button(window, border=0, image= img_reset, activebackground='#00bfff', bg='#00bfff', command=reset_game)
